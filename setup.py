"""A setuptools based setup module for PyHatch

See:
http://pyhatch.readthedocs.org/en/latest/
https://github.com/sonofeft/PyHatch

If installing from source, then
the best way to install PyHatch is to use pip after navigating to the source directory::
...
target_file = path.join( here, 'pyhatch','_version.py')
...
setup(
    ...
    download_url='https://github.com/sonofeft/PyHatch',    
    ...
    entry_points={
        'console_scripts': [
            'pyhatch=pyhatch.hatch_gui:main',
        ],
    },
)